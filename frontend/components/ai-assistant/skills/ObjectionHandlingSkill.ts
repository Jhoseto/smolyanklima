...
export class ObjectionHandlingSkill {
  ...
  private handlePriceObjection(query: string, userContext?: UserContext): string {
    return `💰 **Разбирам напълно** - инвестицията е сериозна. Нека Ви покажа дългосрочната сметка:\n\n` +
      ...
      `• Стар/некачествен климатик: ~150 лв/месец\n` +
      `• Инверторен климатик A+++: ~80 лв/месец\n` +
      `• **Спестяване: 70 лв/месец = 840 лв/година** 💡\n\n` +
      `**📈 Връщане на инвестицията:**\n` +
      `• При цена 1200 лв: изплаща се за ~1.4 години\n` +
      `• След това: чиста печалба от 840 лв/година\n\n` +
