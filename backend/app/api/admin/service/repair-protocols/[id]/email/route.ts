...
export async function POST(
  ...
  const safePdfName = `servizen-protokol-${data.protocol_number.replace(/[^\w.-]+/g, "_")}.pdf`;

  const result = await sendResendEmail({
    to:      parsed.data.email,
    from:    process.env.NOTIFY_EMAIL_FROM ?? "noreply@smolyanklima.com",
    subject: `Сервизен протокол № ${data.protocol_number}`,
    html: `
      <div style="font-family:Arial,Helvetica,sans-serif;max-width:600px;margin:0 auto;color:#111827;">
        ...
        <h2 style="margin:0 0 14px;font-size:18px;color:#0f172a;">Сервизен протокол</h2>
        <p style="margin:0 0 12px;line-height:1.5;">Уважаеми <strong>${clientName}</strong>,</p>
        <p style="margin:0 0 12px;line-height:1.5;">
          Прилагаме сервизния протокол <strong>№ ${protoNo}</strong>
          от <strong>${escHtml(formattedDate)}</strong> за извършения сервиз / профилактика
    ...
    `,
    text:
      `Смолян Клима — Сервизен протокол № ${data.protocol_number} от ${formattedDate}. ` +
