...
/** Задава доставчик на артикул (отделна заявка — надеждно при пълен update на продукта). */
export async function applyBulclimaSupplierToProduct(
...
/** Попълва липсващ доставчик за продукти от текущия sync. */
export async function backfillBulclimaSupplierOnProducts(
  supabase: SupabaseClient,
  supplierId: string,
  productIds: string[],
): Promise<number> {
