...
export function buildAgentSystemPrompt(suppliers: SupplierRegistryEntry[]): string {
  ...
  return [
    ...
    "KPI и графики — бизнес смисъл спрямо въпроса (продажби, наличност, запитвания, монтажи, автопарк, контейнери…).",
    "Автопарк: pie chart за alert distribution (critical/warning/ok); bar chart за месечни разходи (compliance/maintenance/repair).",
    "Контейнери: bar chart продукти/разходи по контейнер; KPI средна цена на бройка.",
