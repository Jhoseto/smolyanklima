...
export function planAutoTools(message: string): AutoToolPlan[] {
  ...
  // --- Fleet ---
  if (/автопарк|fleet|машин|машини|(?<![а-я])кол(?![а-я])|(?<![а-я])коли(?![а-я])|мотор|мпс|винетк|гражданск|отговорност|каско|техническ(?:и|о)\s*преглед|преглед.*(?:машин|мпс|авто|автопарк|кол)|одomet|км\b/.test(m)) {
